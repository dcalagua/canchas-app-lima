...
$('btnGuardar').addEventListener('click',async function(){var err=$('errGuardar');err.style.display='none';if(subiendo>0){pcgToast('Espera a que termine de subir el logo.');return}
  ...
  this.disabled=true;try{var r=await fetch('/anfitrion/campeonatos/guardar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});var j=await r.json();
    if(j.ok){location.href='/anfitrion/campeonatos/'+encodeURIComponent(j.id)+(CFG.nuevo?'?creado=1':'?guardado=1');return}err.textContent=j.error||'No se pudo guardar.';err.style.display='block';if(j.paso)ir(j.paso)}catch(e){err.textContent='No se pudo guardar. Revisa tu conexión.';err.style.display='block'}this.disabled=false});
})();
"""
...
async function post(ruta,body){var r=await fetch(B+ruta,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body||{})});var j=await r.json().catch(function(){return {}});if(!j.ok)throw new Error(j.error||j.mensaje||'No se pudo.');return j}
...
async function subir(file,tipo,M){var blob=await comprimir(file,M);var r=await fetch(B+'/foto?tipo='+tipo,{method:'POST',body:blob,headers:{'Content-Type':'image/jpeg'}});return r.json()}
$('inLogo').addEventListener('change',async function(){var f=this.files&&this.files[0];this.value='';if(!f)return;pcgToast('Subiendo logo…');try{var j=await subir(f,'logo',600);if(j.ok){location.reload();return}pcgToast(j.error||'No se pudo subir.')}catch(e){pcgToast('No se pudo subir el logo.')}});
$('inAusp').addEventListener('change',async function(){var f=this.files&&this.files[0];this.value='';if(!f)return;$('auspMsg').textContent='Subiendo…';try{var j=await subir(f,'ausp',800);if(j.ok){location.reload();return}$('auspMsg').textContent=j.error||'No se pudo subir.'}catch(e){$('auspMsg').textContent='No se pudo subir.'}});
$('inFotos').addEventListener('change',async function(){var files=Array.prototype.slice.call(this.files||[]);this.value='';for(var i=0;i<files.length;i++){$('fotosMsg').textContent='Subiendo foto '+(i+1)+' de '+files.length+'…';try{var j=await subir(files[i],'foto',1400);if(!j.ok){$('fotosMsg').textContent=j.error||'No se pudo subir.';return}}catch(e){$('fotosMsg').textContent='No se pudo subir.';return}}location.reload()});
document.addEventListener('click',async function(ev){var b=ev.target.closest('[data-quitar-img]');if(!b)return;var url=b.closest('.foto').dataset.url;if(!confirm('¿Quitar esta imagen?'))return;try{await post('/imagen/quitar',{tipo:b.dataset.quitarImg,url:url});location.reload()}catch(e){pcgToast(e.message)}});
...
$('btnFondo').addEventListener('click',function(){var r=Math.floor(Math.random()*1e6);
  ...
  $('arte').addEventListener('click',async function(ev){var img=ev.target.closest('img');if(!img)return;try{await post('/afiche',{variante:+img.dataset.v,tema:tema});pcgToast('Fondo elegido');location.reload()}catch(e){pcgToast(e.message)}});
  $('inFondo').addEventListener('change',async function(){var f=this.files&&this.files[0];this.value='';if(!f)return;$('fondoMsg').textContent='Subiendo…';try{var j=await subir(f,'fondo',1600);if(j.ok){location.reload();return}$('fondoMsg').textContent=j.error||'No se pudo subir.'}catch(e){$('fondoMsg').textContent='No se pudo subir.'}});
  var q=$('btnQuitarFondo');if(q)q.addEventListener('click',async function(){try{await post('/afiche',{quitar:true});location.reload()}catch(e){pcgToast(e.message)}});
  pintar()});
// participantes
$('btnAgregar').addEventListener('click',function(){var eq=CFG.deporte==='futbol';
  modal(eq?'Nuevo equipo':'Nuevo participante',"<label>"+(eq?'Nombre del equipo':'Nombre (jugador o pareja "A / B")')+"</label><input id='pNombre' type='text' maxlength='80'><label>WhatsApp <span class='req'>opcional</span></label><input id='pTel' type='text' inputmode='tel' maxlength='20'>"+pie('Agregar'),
    async function(){try{await post('/participante',{nombre:$('pNombre').value,contacto:$('pTel').value});location.reload()}catch(e){err(e.message)}})});
document.addEventListener('click',async function(ev){var q=ev.target.closest('[data-quitar]');if(q){ev.stopPropagation();if(!confirm('¿Quitar a este participante?'))return;try{await post('/participante/'+encodeURIComponent(q.dataset.quitar)+'/eliminar');location.reload()}catch(e){pcgToast(e.message)}return}
...
var bf=$('btnFixture');if(bf)bf.addEventListener('click',async function(){if(CFG.participantes.length<2){pcgToast('Agrega al menos 2 participantes.');return}
  if(CFG.fixture&&!confirm('Regenerarlo BORRA los resultados cargados. ¿Continuar?'))return;try{await post('/fixture');location.reload()}catch(e){pcgToast(e.message)}});
// resultado
document.addEventListener('click',function(ev){var t=ev.target.closest('.partido.clic');if(!t)return;
  modal('Cargar resultado',"<div class='row' style='grid-template-columns:1fr 1fr;gap:10px'><div><label>"+esc(t.dataset.a)+"</label><input id='mA' type='number' min='0' inputmode='numeric' value='"+esc(t.dataset.ma)+"'></div><div><label>"+esc(t.dataset.b)+"</label><input id='mB' type='number' min='0' inputmode='numeric' value='"+esc(t.dataset.mb)+"'></div></div>"+pie('Guardar'),
    async function(){var a=$('mA').value,b=$('mB').value;if(a===''||b===''){err('Escribe ambos marcadores.');return}try{await post('/resultado',{partido:t.dataset.pid,a:+a,b:+b});location.reload()}catch(e){err(e.message)}})});
var br=$('btnRanking');if(br)br.addEventListener('click',async function(){try{var j=await post('/ranking');pcgToast(j.n?j.n+' partido(s) sumado(s) al ranking de la academia.':'Aún no hay partidos jugados para sumar.')}catch(e){pcgToast(e.message)}});
// natación
var bp=$('btnPrueba');if(bp)bp.addEventListener('click',function(){
  modal('Agregar prueba',"<label>Distancia</label><div class='chips' id='dist'>"+CFG.distancias.map(function(d){return "<button type='button' class='chip"+(d===50?' sel':'')+"' data-v='"+d+"'>"+d+" m</button>"}).join('')+"</div><label>Estilo</label><div class='chips' id='est'>"+CFG.estilos.map(function(s){return "<button type='button' class='chip"+(s==='Libre'?' sel':'')+"' data-v='"+esc(s)+"'>"+esc(s)+"</button>"}).join('')+"</div>"+pie('Agregar prueba'),
    async function(){var d=$('dist').querySelector('.sel').dataset.v,s=$('est').querySelector('.sel').dataset.v;try{await post('/prueba',{distancia:+d,estilo:s});location.reload()}catch(e){err(e.message)}});
...
document.addEventListener('click',async function(ev){var q=ev.target.closest('[data-quitar-prueba]');if(q){if(!confirm('¿Eliminar esta prueba y sus tiempos?'))return;try{await post('/prueba/'+encodeURIComponent(q.dataset.quitarPrueba)+'/eliminar');location.reload()}catch(e){pcgToast(e.message)}return}
  ...
  modal('Tiempo de '+m.querySelector('.nom').firstChild.textContent.trim(),"<label>Tiempo (mm:ss.cc)</label><input id='tT' type='text' placeholder='ej. 0:37.85' value='"+esc(m.dataset.t)+"'><div class='row' style='grid-template-columns:1fr 1fr;gap:10px'><div><label>Serie</label><input id='tS' type='number' min='0' value='"+esc(m.dataset.s)+"'></div><div><label>Carril</label><input id='tC' type='number' min='0' value='"+esc(m.dataset.c)+"'></div></div><label style='display:flex;gap:8px;align-items:center'><input type='checkbox' id='tD' style='width:auto;margin:0'"+(m.dataset.dsq==='1'?' checked':'')+"> Descalificado (DSQ)</label>"+pie('Guardar'),
    async function(){try{await post('/marca',{prueba:m.dataset.prueba,participante:m.dataset.pid,tiempo:$('tT').value,serie:+$('tS').value||0,carril:+$('tC').value||0,dsq:$('tD').checked});location.reload()}catch(e){err(e.message)}})});
// duplicar / eliminar
$('btnDuplicar').addEventListener('click',async function(){if(!confirm('¿Crear una nueva edición con los mismos datos (sin participantes ni resultados)?'))return;try{var j=await post('/duplicar');location.href='/anfitrion/campeonatos/'+encodeURIComponent(j.id)+'/editar'}catch(e){pcgToast(e.message)}});
$('btnEliminar').addEventListener('click',async function(){if(!confirm('¿Eliminar este campeonato? No se puede deshacer.'))return;try{await post('/eliminar');location.href='/anfitrion/campeonatos?eliminado=1'}catch(e){pcgToast(e.message)}});
})();
"""
